...
## Integrating the EEF evidence base

The Live Coach should not cite research as a long explanation. It should use the EEF Inclusive Teaching Guide as a compact decision layer:

1. Diagnose the barrier and check-state.
2. Select one EEF principle that best fits the situation.
3. Translate that principle into one small classroom adaptation.
4. Keep cognitive demand high while removing access, language, confidence, or routine barriers.
5. Tell the teacher what to check next.

For example, if EAL pupils are silent because the English wording is risky, the response should prefer `check_3` (reduce social pressure, not cognitive challenge) or `adapt_3` (small adaptations such as vocabulary pre-teaching or adjusted scaffolding). If weaker readers cannot start a written task, the response should separate reading demand from task demand and prefer `adapt_1` or `adapt_2` before lowering the learning goal.
