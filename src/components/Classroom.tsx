...
export default function Classroom({ engagementScore, lastDelta }: { engagementScore: number, lastDelta: number, assets?: any }) {
  ...
  return (
    <div className="relative flex-1 w-full h-full overflow-hidden">
      {/* Base classroom layer - warm, inviting by default */}
      ...
      {/* Disengaged overlay - grays, dims, sad atmosphere */}
      <motion.div 
        className="absolute inset-0 bg-gradient-to-t from-gray-900/60 via-gray-800/40 to-gray-900/60"
        initial={{ opacity: 0 }}
        animate={{ opacity: beastOpacity }}
        transition={{ duration: 1 }}
      />

      {/* Highly engaged overlay - bright, glowing, energized */}
      <motion.div 
        className="absolute inset-0 bg-gradient-to-t from-emerald-900/30 via-yellow-200/20 to-amber-500/30"
        initial={{ opacity: 0 }}
      ...
      {scenarioData.hotspots.map((hotspot) => (
        <div
          key={hotspot.id}
        ...
        >
          ...
          <div className={`w-full h-full rounded-full flex items-center justify-center text-2xl ${
            ...
            engagementScore < 30 ? 'bg-gray-400' : 'bg-blue-400'
          }`}>
